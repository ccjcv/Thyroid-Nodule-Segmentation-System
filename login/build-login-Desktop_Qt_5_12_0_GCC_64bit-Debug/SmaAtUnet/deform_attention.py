...
class DeformableAttention2D(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.pos_gate = nn.Parameter(torch.tensor(1.0), requires_grad=False)

        self.m1 = nn.AdaptiveAvgPool2d(output_size=(1, 1))
        self.m2 = nn.AdaptiveAvgPool2d(output_size=(2, 2))
        self.m3 = nn.AdaptiveAvgPool2d(output_size=(4, 4))
    ...
    def forward(self, x, return_vgrid = False):
        ...
        q = q * self.scale#(16,512,16,16)

         # split out heads

        q, k, v = map(lambda t: rearrange(t, 'b (h d) ... -> b h (...) d', h = heads), (q, k, v))
        #q(16,8,256,64),k(16,8,21,64),v(16,8,21,64)
        # query / key similarity

        sim = einsum('b h i d, b h j d -> b h i j', q, k)#(16,8,256,21)
        ...
        rel_pos_bias = torch.mul(rel_pos_bias, self.pos_gate)
