...
class Aspp_Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        x_shu = self.pool_shu(x_)
        x_shu = self.sr(x_shu)
        x_shu = x_shu.view(b, 16, c)

        x_heng = self.pool_heng(x_)
        x_heng = self.sr(x_heng)
        x_heng = x_heng.view(b, 16, c)
        x = torch.cat((x_1, x_2, x_3, x_4, x_shu, x_heng), dim = 1)#(8,85,512)
        # x = self.sr(x)  # (8,512,8,8)
        # x = rearrange(x, 'b c h w -> b (h w) c')  # (8,64,512)
        x = self.norm(x)
        x = self.act(x)
...
class UNet_bot_aspp_att(nn.Module):
    def __init__(self,
                 n_channels: int = 3,
                 n_classes: int = 2,
                 bilinear: bool = True):
        super(UNet_bot_aspp_att, self).__init__()
        ...
        self.aspp_att = Aspp_Attention(512)
    ...
    def forward(self, x):
        ...
        x5 = self.down4(x4)
        #x5进行self-attention
        x5_aspp_att = self.aspp_att(x5)
        x = self.up1(x5_aspp_att, x4)
        x = self.up2(x, x3)
        x = self.up3(x, x2)
        x = self.up4(x, x1)
        logits = self.outc(x)
