...
class tiao_chihua_module(nn.Module):
    ...
    def forward(self, x):
        ...
        return x_tiao
class UNet_4enc_2chihua_bian(nn.Module):
    def __init__(self,
                 n_channels: int = 3,
                 n_classes: int = 2,
                 bilinear: bool = False):
        super(UNet_4enc_2chihua_bian, self).__init__()
        ...
        # self.fang_chihua_2 = fang_chihua_module_16(dim=64)
        # self.tiaochihua_2 = tiao_chihua_module(dim=64)
        # self.conv1_2 = nn.Conv2d(64, 1, kernel_size=1)
        # # self.up_bian_2 = nn.Upsample(scale_factor=2, mode="bilinear", align_corners=True)
    ...
    def forward(self, x):
        ...
        # x1_fang_chihua = self.fang_chihua_2(x1)
        # x1_tiao_chihua = self.tiaochihua_2(x1)
        # # x1_chihua = self.a5_1 * x1_fang_chihua + self.a5_2 * x1_tiao_chihua
        # x1_chihua = x1_fang_chihua
        # x1_conv1 = self.conv1_2(x1_chihua)
        # # x1_bian = self.up_bian_2(x1_conv1)  # 用来作监督的
        # x1_bian = x1_conv1
        # x1_cheng = torch.mul(x1_tiao_chihua, x1_conv1)
        # x1_jia = x1_cheng + x1  # Boundary Enhanced Feature
